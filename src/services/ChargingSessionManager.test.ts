...
import type { ChargingSession } from '../models';
...
describe('ChargingSessionManager', () => {
  ...
  describe('getSessionStatus', () => {
    ...
    it('should fetch from backend if not in local cache', async () => {
      ...
      const session = await manager.getSessionStatus('session-999');

      expect(session.sessionId).toBe('session-999');
      expect(session.energyDelivered).toBe(5.5);
      expect(fetch).toHaveBeenCalledWith('/api/sessions/session-999');
    });
  ...
  describe('stopSession', () => {
    it('should stop active session and return summary with cost breakdown', async () => {
      ...
      expect(summary.stationName).toBe('Test Station');
      ...
      expect(summary.pricePerKwh).toBe(0.35);
    ...
    it('should calculate correct costs with energy delivered', async () => {
      ...
      const expectedSubtotal = Math.round(summary.energyDelivered * 0.50 * 100) / 100;
    ...
    it('should use default station details if fetch fails', async () => {
      ...
      // Mock station details fetch failure
      (global.fetch as any).mockRejectedValueOnce(new Error('Network error'));
    ...
    it('should continue even if persist stopped session fails', async () => {
      ...
      // Mock persist stopped session failure
      (global.fetch as any).mockRejectedValueOnce(new Error('Network error'));
